---
name: metaplan
description: Write a "metaplan" planning notebook — a long-form Markdown doc for a multi-slice project that tracks scope, decisions, requirements, risks, and grounded research facts with cross-referenced IDs. Use when the user asks to write up a project plan, planning notebook, or "metaplan" doc, especially one spanning multiple tickets/PRs where decisions and raw research need to be preserved for later reference. Also use when bootstrapping a new metaplan from a vague idea. Use this skill every time you work on a `*.metaplan.md` file, or a Markdown file that has a work plan with tickets.
---
...
1. Ask the user for the subject matter. Derive a slug, create `~/.notebooks/<yy><mm>-<slug>/index.metaplan.md`.
